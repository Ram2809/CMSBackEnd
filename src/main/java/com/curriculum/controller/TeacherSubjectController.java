...
public class TeacherSubjectController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<Response> updateTeacherSubjectAssign(@PathVariable("id") Long id,@RequestBody TeacherSubject teacherSubject) throws TeacherNotFoundException, SubjectNotFoundException
	{
		Response response=new Response();
		ResponseEntity<Response> responseEntity=null;
		...
		try {
			...
			if(e instanceof TeacherNotFoundException || e instanceof SubjectNotFoundException || e instanceof AssignIdNotFoundException)
			{
				response.setCode(404);
				response.setMessage(e.getMessage());
				responseEntity=new ResponseEntity<>(response,new HttpHeaders(),HttpStatus.NOT_FOUND);
